...
async def _log_done(job_id: str, user_id: int | None, started: float, card_count: int) -> None:
    ...
    if final and final["status"] == JobStatus.ERROR:
        if user_id:
            try:
                await db.refund_free_deck(user_id)
            except Exception:
                logger.exception("무료 체험 환불 실패 (job=%s, user_id=%s)", job_id, user_id)
        # 크레딧 선차감 환불 — jobs.charged_credits 각인 기반(멱등, recover_stale_jobs와 동일 훅, §13-①).
        try:
            await db.refund_job_credits(job_id)
        except Exception:
            logger.exception("크레딧 환불 실패 (job=%s)", job_id)
    try:
        await db.log_event(
