...
- 폰트는 CDN <link>로 불러옵니다. 본문=Pretendard. 여기에 **디스플레이 서체를 반드시 하나 더**
  ...
  기술적=강한 산세리프). 수치·라벨은 등폭(JetBrains Mono 등). **서체 대비로 위계**를 만드세요.
...
- **빛나는 초점**: 주제를 은유하는 초점 요소를 CSS만으로 발명(구체·파동·격자·단면 등, 논문에 맞게).
  radial-gradient + inset shadow + 은은한 글로우. 표지에 하나 두면 강하다. 단 매번 '구체'로 고정하지 말 것.
- **밝음↔어둠 페이싱**: 밝은 카드와 어두운 카드를 번갈아 배치해 7장에 스크롤 리듬을 준다.
- **타이포 위계**: 디스플레이 서체(헤드라인) + Pretendard(본문) + 등폭(수치) 3단 대비.
- **데이터는 DOM으로**: 막대·비교·게이지를 flex와 height %로 그리되, 높이가 실제 수치 비율과 정확히 일치.
- **큰 수치가 주인공**: 핵심 숫자 하나를 등폭 대형으로 키워 카드의 중심에.
- **절제**: 장식은 의미가 있을 때만. 과하면 촌스럽다.
