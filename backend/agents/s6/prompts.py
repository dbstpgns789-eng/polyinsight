...
- 그렇다 → 역할이 '문제 제기'든 '핵심 혁신'이든 상관없이 항목형 뼈대를 우선한다
  ...
  ★단, 반대로 하나의 개념·기법을 억지로 여러 항목으로 쪼개지도 마라(예: MLM=마스킹+맞히기는 한 기법).
   진짜로 독립된 항목인지 의미 단위로 확인하라 — 쪼개면 그 분야를 모른다는 인상을 준다.
...
I. ★제목=인사이트, 본문=근거 (스포일러·동어반복 금지): headline은 '그래서 무엇이 중요한가'라는
   인사이트/결론을 말하고, 항목·본문은 그 근거(숫자·사실)를 담는다. 제목에 본문의 숫자를 그대로 나열하면
   독자가 본문을 읽을 이유가 사라진다.
   나쁨: 제목 "29.4M 위치, 30M 게임, 50 GPU, 3주" + 박스에 같은 숫자 4개(동어반복)
   좋음: 제목 "압도적 *컴퓨팅 파워*가 만든 훈련 격차" + 박스에 29.4M·50 GPU·3주(근거)
J. ★제목 수량어 일치: 제목에 "두 가지/3가지/네 축" 같은 수를 쓰면 실제 항목 수와 반드시 일치시킨다
   ("두 가지 학습법"인데 1·2·3 세 개로 나열 금지).
K. ★개념 무결성: 하나의 기법·개념을 여러 항목으로 쪼개지 마라. 항목들은 서로 독립이어야 한다.
   예: MLM(마스크 후 맞히기)은 한 기법 — "마스킹"과 "문맥으로 맞히기"를 별개 항목으로 분리 금지.
   쪼개면 그 분야를 모른다는 인상을 준다 — 의미 단위로 묶어라.
