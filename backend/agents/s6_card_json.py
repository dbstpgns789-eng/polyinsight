...
C. 각 카드는 메시지 하나. 문장은 짧고 구어체로 (종결어미: ~다 대신 ~네요, ~이에요 사용 가능)
   - 한 카드에 주장 2개 이상 우겨넣지 않는다. 부연은 과감히 버린다.
   - 글자 수 상한(공백 포함, *별표* 제외): headline ≤ 30자, subtitle ≤ 45자, body는 짧은 2~3문장 ≤ 90자. 넘으면 더 풀어쓰지 말고 압축한다.
D. 전문 용어를 쓸 때는 독자가 이해할 수 있는 한 줄 설명을 괄호 안에 붙인다
E. 첫 카드(cover_v2)의 headline = 스크롤을 멈추는 훅. 추상 제목·기관 자랑·인사말 금지.
   손실회피("~하면 손해") 또는 구체적 약속·숫자("OO% 향상", "OO하는 3가지")로 '끝까지 볼 이유'를 1초 안에 준다.
F. 마지막 카드(closing_v2)는 단순 마무리가 아니라 앞 카드 핵심을 압축한 '한 장 요약'.
