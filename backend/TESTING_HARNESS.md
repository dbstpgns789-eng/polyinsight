...
| 지표 | 목표 | 실측 (2026-06-26) |
|------|------|-------------------|
| S1 추출 성공률 | > 98% (85편 중 83편 이상) | **98.8%** (84/85) ✅ |
| s1_no_sections 비율 | < 5% | **1.2%** (1편) ✅ |
| 실행 시간 (85편) | < 10분 | ~11분 (순차모드) △ |

**실패 논문 (핀셋 대상)**:
- `생분해성 플라스틱 분해 조절을 위한 미생물 및 효소 기술.pdf`
- columns=2 · journal=other:10.7317
- 원인: 한국어 2컬럼 논문, 한국어 섹션명 미인식 추정 → `s1_no_sections`

> 실행 시간 11분(순차모드): pymupdf4llm이 페이지당 순차 처리. 병렬 Pool 모드
> 기준 측정 미완 (Windows multiprocessing 우회로 순차모드 사용). 목표 10분은
> Pool 4-worker 기준으로 재측정 필요.
