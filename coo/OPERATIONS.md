...
- [ ] **5) 이메일 실발송 살리기**. 지금 Resend 테스트모드라 **박사님 인증메일이 안 감 → 로그인 못 할 수 있음.** 도메인 DKIM 검증 후 `EMAIL_FROM` 교체하거나, 소수 파일럿이면 계정을 DB에서 미리 인증처리(grace로 덱 3개는 인증 없이 가능). (상세 §10-1)
...
### 10-1. ★이메일 실발송 (배포 전 필수 — 안 하면 로그인 자체가 막힘)
지금 Resend가 **테스트모드**(`EMAIL_FROM=onboarding@resend.dev`, `config.py`) = 사실상 **가입자 본인 주소로만** 발송 + 도메인 DKIM 미검증. 박사님/외부 사용자의 **가입 인증메일·비번재설정 메일이 안 도착**한다.
- **소수 지인 파일럿**: 박사님 계정을 내가 미리 만들고 DB에서 인증처리(`email_verified=1`) → 이메일 없이 바로 씀. (미인증도 grace로 덱 3개까지는 가능)
- **제대로**: 도메인 사서 Resend DKIM 검증 → `EMAIL_FROM` 그 도메인으로 교체 → **외부 주소로 가입해 인증메일 실제 도착 1회 확인.**
