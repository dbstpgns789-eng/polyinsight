...
## 📡 세션 간 조정 메모 (2026-07-20 11:20, 리뷰 세션 → 실행 세션)

> **두 세션이 같은 레포에서 동시에 작업 중이다.** 실시간 채널이 없으므로 이 배너가 유일한 조정 수단이다.
> **다음 태스크를 시작하기 전에 이 절을 다시 읽고, `git log --oneline -5`로 상대 커밋을 확인할 것.**

**리뷰 세션이 한 일 (커밋 `2d4719d`):**
- **Task 12 신설** — 벽 히트(402) 계측. 지금 플랜대로 끝내면 벽은 서지만 **몇 명이 부딪혔는지 셀 수가 없다.** BM 로드맵상 T1(결제 켜기) 트리거가 "벽 히트 30~50명"이라 이 숫자가 없으면 다음 단계 판단이 불가능하다.
- **Task 13 신설** — 무료 유저 인라인 카드 축소 서빙. 최초 플랜이 "알려진 구멍"으로 남겨둔 원본 해상도 유출을 막는다. 렌더 파이프라인은 안 건드려도 된다(서빙 레이어 리사이즈, Pillow는 기존 의존성).
- 상단 "이 플랜이 하지 않는 것" 표와 File Structure 표도 함께 갱신됨.

**실행 세션이 한 일 (리뷰 세션이 확인함):**
- `7aa8f8a` 원자적 소비로 **동시 업로드 레이스를 스스로 해소함** — 리뷰 세션도 같은 결함을 찾았으나, 실행 세션 쪽 처리(호출부 `plan_of(user)=='free'` 가드 포함)가 더 나아 손대지 않았다. **이 건은 종결.**
- `00f2bd0` Task 2(`plans.py`) 완료.

**★충돌 주의 — Task 3 시작 전 필독:**
1. **`plans.py`를 Task 12에서 한 번 더 연다.** Task 12 Step 2가 `HTTPException` → `PlanGateError`로 승격한다. Task 2를 이미 커밋했으므로, **Task 12를 Task 3보다 먼저 실행**하거나, Task 3에서는 평범한 402로 두고 Task 12에서 일괄 교체할 것.
2. **Task 3의 `plans.author_gate_error()` 호출부는 `user`를 받아야 한다** — `raise plans.author_gate_error(user)`. 안 넘기면 레이스에서 진 요청의 벽 히트가 익명으로 남아 퍼널 집계에서 빠진다.
3. **Task 13의 두 경로(`get_card_image`·`get_deck_card`)에 402를 걸지 말 것.** Task 5에서 export 4경로에 게이트를 걸 때 이 둘을 함께 잡고 싶은 유혹이 있는데, 걸면 무료 유저 뷰어가 빈 화면이 되고 아하가 죽는다. 이 둘은 **해상도 차등**만 적용한다.

**작업 분담 제안:** 백엔드(Task 3~6·12·13)는 실행 세션이 이어서 소유한다. 리뷰 세션은 읽기만 한다. 이 배너를 갱신할 때는 **누가 언제 썼는지** 남길 것.
