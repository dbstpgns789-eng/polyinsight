// Ticker(skeleton) — 지표 전광판(skin Ticker) + 출처. 피부만 조립. 등록키 'ticker'.
'use client'
import { CardSurface, BrandMark, Eyebrow, Headline, Ticker, SourceTag, parseTableRows, tableRowsToRaw } from '../skin'
import type { CardComponentProps } from '../types'
import { fieldValue } from '../types'

export default function TickerCard(props: CardComponentProps) {
  const { card, mode, onFieldChange, onFieldFocus, focusedField } = props
  const items = parseTableRows(fieldValue(card, 'items'))
  const onItem = (i: number, field: 'attr' | 'a' | 'b', text: string) => {
    const next = items.map((r, idx) => (idx === i ? { ...r, [field]: text } : r))
    onFieldChange?.('items', tableRowsToRaw(next))
  }
  return (
    <CardSurface>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', marginTop: 28 }}>
        <Ticker items={items} mode={mode} onItemChange={onItem}
