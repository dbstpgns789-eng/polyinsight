...
colors:
  ...
  forest-green:        "oklch(63% 0.13 163)"     # 브랜드 주색·CTA·검증. jewel emerald ≈#0FA37B
  forest-green-deep:   "oklch(56% 0.12 164)"     # CTA hover·강조. ≈#0B8A67
  forest-green-bright: "oklch(73% 0.14 162)"     # 다크 위 액센트·그라디언트 끝·해자 강조. ≈#13B08A / #4FD39A
  forest-green-wash:   "oklch(95% 0.03 163)"     # 검증 배지 배경·선택 하이라이트. ≈#E1F4EC
  forest-green-ghost:  "oklch(97.5% 0.015 163)"  # 가장 약한 hover
  cta-gradient:        "linear-gradient(90deg, oklch(63% 0.13 163), oklch(73% 0.14 162))"  # 브랜드 CTA (≈#0FA37B→#13B08A)
  canvas:              "oklch(99.4% 0.002 163)"
  canvas-warm:         "oklch(98% 0.004 163)"
  canvas-subtle:       "oklch(96% 0.006 163)"
  canvas-muted:        "oklch(93% 0.008 163)"
  border:              "oklch(91% 0.008 163)"
  border-subtle:       "oklch(94% 0.005 163)"
  ink-1:               "oklch(19% 0.01 163)"
  ink-2:               "oklch(46% 0.012 163)"
  ink-3:               "oklch(64% 0.008 163)"
  dark-slate:          "oklch(30% 0.06 163)"     # 딥그린 해자/다크 섹션 배경 ≈#0C3A2E
  dark-slate-2:        "oklch(34% 0.06 163)"     # 다크 섹션 내부 서피스 ≈#10473A
  ...
  status-ok:           "oklch(57% 0.12 163)"     # 검증 완료 = 브랜드 에메랄드
  status-ok-bg:        "oklch(95% 0.03 163)"
typography:
  display:
...
색상은 절제되지만 죽지 않는다. 에메랄드는 전형적인 SaaS 블루/퍼플을 의도적으로 거부하며, "검증된 초록"이라는 우리만의 논리에서 나온 색이다(장식이 아니다). 헤드라인은 잉크(near-black)로 무게를 지키고, 에메랄드는 로고·CTA·검증 배지에서 정체성을 말한다. 딥그린 해자 섹션(`dark-slate`)이 이 정체성을 다크 서피스로 확장한다. 모든 중성색은 hue 163로 미세하게 틴팅되어 — 순수한 회색이 아닌, 브랜드 색조가 스며든 화이트와 그레이 — 전체 인터페이스에 일관된 온도감을 준다.
...
**Key Characteristics:**
- 모든 색상은 OKLCH, hue 163로 일관 틴팅 (검정도, 흰색도 순수하지 않다)
...
브랜드 색은 hue 163(에메랄드)를 공유한다. 중성색도 hue 163로 미세 틴팅. 위험도 색만 의도적으로 다른 hue(25/50/80). CTA는 에메랄드 그라디언트(`cta-gradient`) 허용 — 단, 그라디언트 텍스트는 금지.

### Primary
- **에메랄드** (`oklch(63% 0.13 163)`): 브랜드 주색이자 검증색(통합). 로고, CTA, 링크, 활성 상태, 진행 표시, 검증 배지. jewel emerald ≈#0FA37B.
- **에메랄드 딥** (`oklch(56% 0.12 164)`): CTA hover·강조. 독립적으로 등장하지 않는다.
- **에메랄드 브라이트** (`oklch(73% 0.14 162)`): 딥그린 다크 위 액센트·그라디언트 끝·해자 강조 수치(≈#4FD39A).
- **에메랄드 워시** (`oklch(95% 0.03 163)`): 검증 배지 배경, 선택 항목, 인라인 하이라이트.
- **에메랄드 고스트** (`oklch(97% 0.018 163)`): hover 피드백 중 가장 약한 단계.

### Neutral
- **캔버스** (`oklch(99.8% 0.002 163)`): 카드, 모달, 입력 필드의 표면. 순수한 흰색이 아닌, hue 163로 미세 틴팅.
- **캔버스 웜** (`oklch(98% 0.005 163)`): 페이지 배경. 캔버스보다 한 단계 어둡다.
- **캔버스 서틀** (`oklch(95.5% 0.009 163)`): 비활성 섹션, 내부 배경.
- **캔버스 뮤티드** (`oklch(92% 0.012 163)`): 구분선 영역, 입력 비활성 배경.
- **보더** (`oklch(89% 0.012 163)`): 카드 테두리, 인풋 테두리.
- **보더 서틀** (`oklch(93.5% 0.007 163)`): 내부 구분선.

### Text
- **잉크 1** (`oklch(16% 0.008 163)`): 본문 텍스트, 제목. 순수 검정이 아닌 틴팅된 짙은 색.
- **잉크 2** (`oklch(44% 0.012 163)`): 보조 텍스트, 레이블, 메타 정보.
- **잉크 3** (`oklch(63% 0.008 163)`): 비활성 텍스트, 플레이스홀더.

### Dark Layer
- **다크 슬레이트** (`oklch(14% 0.04 163)`): 랜딩 페이지 다크 섹션 배경. hue 163 틴팅이 두드러진다.
- **다크 슬레이트 2** (`oklch(19% 0.04 163)`): 다크 섹션 내부 서피스.
...
- **OK** (`oklch(34% 0.12 163)`): 검증 완료. 브랜드 hue와 같아 — 안전은 브랜드 그린이다.

**The One Accent Rule.** 에메랄드은 주어진 화면의 ≤10%를 차지한다. 그것의 희소성이 신뢰를 만든다. 버튼과 링크가 아닌 곳에 accent color를 사용하는 것은 금지다.

**The Tinted Neutrals Rule.** 순수한 `#000`, `#fff`는 이 시스템에 존재하지 않는다. 모든 중성색은 hue 163로 틴팅된다. 순수 검정/흰색이 등장하면 디자인이 브랜드 팔레트에서 이탈했다는 신호다.
...
### Shadow Vocabulary
- **shadow-sm** (`0 2px 8px oklch(8% 0.01 163 / 0.07)`): 카드 hover 상태. 거의 느껴지지 않는 들어올림.
- **shadow-md** (`0 12px 32px oklch(8% 0.01 163 / 0.10)`): 드롭다운, 툴팁, 선택된 카드.
- **shadow-lg** (`0 16px 48px oklch(8% 0.01 163 / 0.18)`): 업로드 모달.
- **shadow-modal** (`0 24px 64px -12px rgba(15,20,40,0.30)`): 내보내기 모달, 가장 강한 그림자. 배경 오버레이와 함께 사용된다.

모든 그림자는 hue 163로 틴팅된 어두운 값을 사용한다. 순수 검정 그림자(`rgba(0,0,0,...)`)는 팔레트에서 이탈한 신호다.
...
- **Primary:** 에메랄드 배경 + 캔버스 텍스트. 높이 40px, 패딩 10px 24px. Label 타이포(600, 0.75rem). hover 시 forest-green-deep으로 전환 (80ms ease-out).
- **Ghost:** 투명 배경 + 에메랄드 테두리 + 에메랄드 텍스트. hover 시 forest-green-wash 배경.
...
- **Background:** 캔버스 (`oklch(99.8% 0.002 163)`).
- **Border:** 보더 색상 (`oklch(89% 0.012 163)`), 1px solid.
...
- **Overlay:** `oklch(14% 0.04 163 / 0.70)` 딤 + `blur(8px) saturate(120%)` backdrop.
...
- **Focus:** 에메랄드 2px solid outline, outline-offset 0. border-color도 forest-green으로 전환.
...
- 완료 단계: status-ok 체크. 진행 단계: 스피너 + 에메랄드. 대기 단계: 잉크3.
...
### Do:
- **Do** 모든 색상에 OKLCH를 사용한다. 중성색은 반드시 hue 163로 틴팅한다.
...
### Don't:
- **Don't** 순수 `#000` 또는 `#fff`를 사용한다. 모든 검정/흰색은 hue 163로 틴팅되어야 한다.
- **Don't** 에메랄드 액센트를 화면의 10% 이상에 사용한다. 희소성이 신뢰를 만든다.
